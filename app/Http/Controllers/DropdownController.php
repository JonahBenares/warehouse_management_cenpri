...
class DropdownController extends Controller
{
    ...
    public function all_po(){
        ...
        public function notedemp_list(){
            ...
            $rec_noted_pos=User::where('id','31')->value('position');
            $iss_noted_pos=User::where('id','74')->value('position');
            $res_noted_pos=User::where('id','75')->value('position');
            return response()->json([
                'users'=>$users,
                'default_rec_noted_pos'=>$rec_noted_pos,
                'default_iss_noted_pos'=>$iss_noted_pos,
                'default_res_noted_pos'=>$res_noted_pos
            ],200);
        }

        public function acknowledgeemp_list(){
            $users=User::where('name','!=','')->where('acknowledge_flag','1')->orderBy('name','ASC')->get()->unique('name');
            $rec_ack_pos=User::where('id','74')->value('position');
            return response()->json([
                'users'=>$users,
                'default_rec_ack_pos'=>$rec_ack_pos
            ],200);
        }
