								...
								<div  v-for="(det, index) in details" class="border-2 p-2 border-b-0 mt-2">
									...
									<table width="100%" class="table-bordered">
										...
										<tr v-for="(it,index) in det.receive_items.items">
											...
											<td v-if="det.explode_modes[index]!='deduct'" :class="(it.eval_flag!=0) ? (it.eval_flag==1) ? 'text-xs align-top px-1 py-1' : (it.eval_flag==2) ? 'text-xs align-top px-1 py-1' : '' : 'text-xs align-top px-1 py-1'" align="center">{{ ((parseFloat(it.unit_cost) + parseFloat(it.shipping_cost)) *  it.rec_quantity).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</td>
