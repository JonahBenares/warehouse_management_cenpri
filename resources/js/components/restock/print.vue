<script setup>
	...
	const getNoted = async () => {
		...
		if (!head.value.noted_by) {
			head.value.noted_by = 75;
			head.value.noted_by_position = response.data.default_res_noted_pos;
		}
	}
	...
	const getAcknowledge = async () => {
		...
		if (!head.value.acknowledged_by) {
			...
			head.value.acknowledged_by_position = response.data.default_rec_ack_pos;
		}
	}
	...
	const getAcknowledgedby = async () => {
		if (head.value.acknowledged_by !== '') {
			...
			head.value.acknowledged_by_position = response.data;
		} else {
			ack_position.value = '';
			head.value.acknowledged_by_position = '';
		}
	}
	...
	const getNotedby = async () => {
		if (head.value.noted_by !== '') {
			...
			head.value.noted_by_position = response.data;
		} else {
			noted_position.value = '';
			head.value.noted_by_position = '';
		}
	}
</script>
