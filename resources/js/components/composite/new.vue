...
import {  TrashIcon, PlusIcon, XMarkIcon,CheckCircleIcon, ExclamationCircleIcon, ArrowUturnLeftIcon, EyeIcon } from '@heroicons/vue/24/solid'
			...
			<div class="card mb-3">	
				<div class="flex justify-between content-center">
					<div class="flex justify-start space-x-3 ">
						...
						<div>
							<h6 class="m-0 pt-1 font-bold uppercase">Composite Item List</h6>
						</div>
					</div>	
					<div class="pt-1">	
						<nav aria-label="breadcrumb" role="navigation">
							<ol class="breadcrumb adminx-page-breadcrumb">
								...
								<li class="breadcrumb-item"><a href="/item_list/composite">Composite Items List</a></li>
											...
											<table class="table table-bordered mb-0">
												<thead>
													<tr>
														...
														<th class="" width="40%">Variant</th>
														<th class="">Quantity</th>
														<th class="p-1 " align="center" width="1%">
												...
												<tbody>
													<tr v-for="(comp, index) in composite_list">
														<td class="p-0  ">
															...
															<select name="items" id="items" class="form-control focus:outline-none focus:ring-0" v-model="comp.compose_item_id" @change="checkVariant(index)" :disabled="draft==1 && comp.id!=0">
																<option :value="item.id" v-for="item in listitems" :key="item.id">{{ item.item_description }}</option>
															</select>
														</td>
														<td class="p-0  ">
															<select name="variant_comp" id="variant_comp" class="form-control focus:outline-none focus:ring-0" v-model="comp.variant_id" @change="checkVariantqty(index,comp.variant_id,comp.compose_item_id)" :disabled="draft==1 && comp.id!=0">
																<option :value="v.id" v-for="v in variant[index]" :key="v.id">{{ v.supplier_name }}, {{ v.brand }}</option>
															</select>
															<input type="hidden" v-model="comp.pr_no">
														</td>
														<td class="p-0  ">
															<textarea type="number" v-model="comp.quantity" rows="1" class="p-2 m-0 w-full leading-none block " placeholder="Quantity" :disabled="draft==1 && comp.id!=0" @blur="checkMaxqty(index)"></textarea>
