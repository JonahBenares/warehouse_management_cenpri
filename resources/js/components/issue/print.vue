<script setup>
	...
	const getNoted = async () => {
		...
		if (!head.value.noted_by) {
			head.value.noted_by = 74;
			head.value.noted_by_pos = response.data.default_iss_noted_pos;
		}
	}
							...
							<div >
								...
								<table width="100%" class="table-bordered mt-2">
									<tr>
										...
										<td class="text-sm text-center text-sm" width="9%">Item Cost</td>                 
										<td class="text-sm text-center text-sm" width="9%">Total Cost</td>                 
									...
									<tr  v-for="(it,x) in items">
										...
										<td class="text-sm align-top px-1" align="center">{{ parseFloat(it.shipping_cost  + it.unit_cost).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} {{ (it.currency!=null) ? it.currency : '' }}</td>
										<td class="text-sm align-top px-1" align="center">{{ parseFloat((it.shipping_cost  + it.unit_cost) * it.issued_qty).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }} {{ (it.currency!=null) ? it.currency : '' }}</td>
										<td class="text-sm align-top px-1" align="center">{{ parseFloat(it.inventory_balance).toFixed(2) }}</td>
									</tr>
