<script setup>
	...
	const getAcknowledge = async () => {
		...
		// Set default only after list is loaded
		if (!head.value.acknowledged_by) {
			head.value.acknowledged_by = 74;
			head.value.acknowledged_position = response.data.default_rec_ack_pos;
		}
	};
	...
	const getNoted = async () => {
		...
		if (!head.value.noted_by) {
			head.value.noted_by = 31;
			head.value.noted_position = response.data.default_rec_noted_pos;
		}
	}
	...
	const getAckby = async () => {
		if (head.value.acknowledged_by !== '') {
			...
			head.value.acknowledged_position = response.data;
		} else {
			ack_position.value = '';
			head.value.acknowledged_position = '';
		}
	}
	...
	const getNotedby = async () => {
		if (head.value.noted_by !== '') {
			...
			head.value.noted_position = response.data;
		} else {
			noted_position.value = '';
			head.value.noted_position = '';
		}
	}
							...
							<div >
								...
								<div  v-for="(det, index) in details" class="border-2 p-2 border-b-0 mt-2">
									<table class="w-full table-borsdered" >
										...
										<tr>
											<td class="leading-tight text-sm">Supplier</td>
											<td class="leading-tight text-sm" colspan="3"><span class="pr-2">:</span>{{ det.supplier_name }}</td>
										</tr>
									</table>
									<table width="100%" class="table-bordered mt-2">
										<tr class="bg-gray-50">
											<td class="text-sm px-1" width="2%" align="center">#</td>
											<td class="text-sm px-1" width="3%" align="center">Qty</td>
											<td class="text-sm px-1" width="2%" align="center">U/M</td>
											<td class="text-sm px-1" width="10%" align="left">Part No</td>
											<td class="text-sm px-1" width="20%" align="left">Item Description</td>
											<!-- <td class="text-sm px-1" width="20%" align="center">Supplier</td> -->
											<td class="text-sm px-1" width="10%" align="left">Catalog No.</td>
											<!-- <td class="text-sm px-1" width="10%" align="center">Brand</td>
											<td class="text-sm px-1" width="10%" align="center">Color</td>
											<td class="text-sm px-1" width="10%" align="center">Size</td> -->
											<td class="text-sm px-1" width="6%" align="left">Item Status</td>
											<td class="text-sm px-1" width="5%" align="right">S/U Cost</td>
											<td class="text-sm px-1" width="5%" align="right">Cost</td>
											<!-- <td class="text-sm px-1" width="5%" align="center">Shipping Fee</td> -->
											<td class="text-sm px-1" width="5%" align="right">Total Cost</td>
										</tr>
										...
										<tr v-for="it in det.receive_items.items">
											...
											<td class="text-sm align-top px-1" align="center">{{it.item_no}}</td>
											<td class="text-sm align-top px-1" align="center">{{it.rec_quantity}}</td>
											<td class="text-sm align-top px-1" align="left">{{it.uom}}</td>
											<td class="text-sm align-top px-1" align="left">{{it.pn_no}}</td>
											<td class="text-sm align-top px-1" align="left">
												<div class="font-medium text-gray-800">
													{{ it.item_description }}
												</div>
												<div class="mt-.5 text-gray-600 space-x-2 text-xs">
													<span v-if="it.brand"><strong>Brand:</strong> {{ it.brand }}</span>
													<span v-if="it.color"><strong>Color:</strong> {{ it.color }}</span>
													<span v-if="it.size"><strong>Size:</strong> {{ it.size }}</span>
												</div>
											</td>
											<!-- <td class="text-sm align-top px-1" align="left">{{it.supplier_name}}</td> -->
											<td class="text-sm align-top px-1" align="left">{{it.catalog_no}}</td>
											<!-- <td class="text-sm align-top px-1" align="left">{{it.brand}}</td>
											<td class="text-sm align-top px-1" align="left">{{it.color}}</td>
											<td class="text-sm align-top px-1" align="left">{{it.size}}</td> -->
											<td class="text-sm align-top px-1" align="left">{{it.item_status}}</td>
											<td class="text-sm align-top px-1" align="right">{{ it.shipping_cost }}</td>
											<td class="text-sm align-top px-1" align="right">{{it.unit_cost}} {{ (it.currency!=null) ? it.currency : '' }}</td>
											<!-- <td class="text-sm align-top px-1" align="right">12</td> -->
											<td class="text-sm align-top px-1 bg-yellow-50" align="right">{{ ((parseFloat(it.unit_cost) + parseFloat(it.shipping_cost)) *  it.rec_quantity).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}}</td>
										</tr>
										
										<tr>
											<td class="text-sm" colspan='9' align='right'><span class="mx-1"><b>Total</b></span></td>
											<td class="text-sm bg-yellow-200 px-1" colspan='1' align='right'><b>{{ parseFloat(qty).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</b></td>
										</tr>
									</table>
									<table width="100%">
										<tr>
